...
final float   paper_size_y = 600;
final float   image_size_x = 400;
final float   image_size_y = 600;
final float   paper_top_to_origin = 200;      //mm, make smaller to move drawing down on paper
final float   paper_gor_to_origin = 300;      //mm, make smaller to move drawing gorizont on paper
...
final float   grid_scale = 100.0;              // Use 10.0 for centimeters, 25.4 for inches, and between 444 and 529.2 for cubits.
...
pfm ocl;
int current_pfm = 0;
String[] pfms = { "PFM_original", "PFM_spiral" , "PFM_squares" }; //"PFM_spiral" , "PFM_squares" ,
int sidebarWidth = 220; // Ширина боковой панели
int     state = 1;
int     pen_selected = 0;
...
void setup() {
  ...
  cp5.addButton("loadNextPFM")
     .setLabel("Load Next PFM")
     .setPosition(width - sidebarWidth + 10, 20)
     .setSize(200, 40)
...
void loadNextPFM() {
  ...
  // Вызов соответствующей функции из скрипта
}

void rotateDrawing() {
  println("Rotate drawing");
  // Вызов соответствующей функции из скрипта
}
...
void resetDrawing() {
  println("Reset drawing");
  // Вызов соответствующей функции из скрипта
}

void generateGcode() {
  println("Generate G-code");
  // Вызов соответствующей функции из скрипта
}
////////////////////////////////////////////////////
...
void draw() {
  ...
  // Обновляем интерфейсный слой каждый кадр
  uiLayer.beginDraw();
  uiLayer.clear();
  uiLayer.fill(200);
  uiLayer.rect(width - sidebarWidth, 0, sidebarWidth, height);
  uiLayer.endDraw();

  // Рисуем интерфейсный слой поверх всего
  image(uiLayer, 0, 0);

  // Рисуем кнопки поверх интерфейсного слоя
  cp5.draw();
}
